...
describe('HttpServer (integration)', () => {
  ...
  it('applies server properties', async () => {
    ...
    expect(server.server).to.have.property('keepAliveTimeout').which.is.equal(101);
    expect(server.server).to.have.property('headersTimeout').which.is.equal(102);
    expect(server.server).to.have.property('maxConnections').which.is.equal(103);
    expect(server.server).to.have.property('maxHeadersCount').which.is.equal(104);
