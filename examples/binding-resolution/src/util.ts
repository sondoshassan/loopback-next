 ...
 * @param resolutionCtx - Resolution context object
 * @param binding - Current binding
 * @param logger - An optional log function
 */
export function logResolutionContext(
  resolutionCtx: Context,
  binding: Binding<unknown>,
  logger: Logger = debug,
) {
  const ownerCtx = resolutionCtx.getOwnerContext(binding.key);
  logContext('Owner', ownerCtx, binding, logger);
  logContext('Resolution', resolutionCtx, binding, logger);
}
